...
export async function authenticateWithGitHub(context: vscode.ExtensionContext): Promise<GitHubUser | null> {
    let cachedToken = context.globalState.get<string>("sigil-ps_AccessToken");
    ...
    try {
        ...
        if (session) {
            ...
            await context.globalState.update("sigil-ps_UserId", session.account.id);
            await context.globalState.update("sigil-ps_AccessToken", session.accessToken);
